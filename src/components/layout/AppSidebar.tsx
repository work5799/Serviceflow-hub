import { 
  LayoutDashboard, FolderKanban, DollarSign, Layers, Users, 
  Bell, Settings, LogOut, ChevronLeft, Zap
} from 'lucide-react';
  ...
  return (
    <aside className={cn(
      ...
      <div className="flex items-center gap-3 px-4 h-16 border-b border-sidebar-border">
        ...
        {!collapsed && (
          <span className="font-bold text-sidebar-accent-foreground text-lg tracking-tight">AgencyOS</span>
        )}
        <button 
        ...
        <div className={cn("flex items-center gap-3", collapsed && "justify-center")}>
          ...
          {!collapsed && (
            <div className="flex-1 min-w-0">
              ...
              <span className={cn("text-[10px] px-1.5 py-0.5 rounded-full font-medium", ROLE_COLORS[currentUser.role])}>
                {ROLE_LABELS[currentUser.role].split(' ')[0]}
              </span>
            </div>
          )}
          {!collapsed && (
            <button className="p-1.5 rounded-md text-sidebar-foreground hover:bg-sidebar-accent transition-colors">
